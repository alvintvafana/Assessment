...
{
    public class UpdateBookDetailCommandHandler : ICommandHandler<UpdateBookDetailCommand>
    {
        private readonly IRepository<Book> _repository;
        private readonly IUnitOfWork _uow;
        public UpdateBookDetailCommandHandler(IRepository<Book> repository, IUnitOfWork uow)
        {
            _repository = repository;
            _uow = uow;
        }
        public async Task HandleAsync (UpdateBookDetailCommand command)
        {

            var book = await _repository.GetByIDAsync(command.Id);
            if(book==null)
                throw new ValidateException("book doesnot exists");
            book.UpdateBookDetail(command.Name, command.Text);
